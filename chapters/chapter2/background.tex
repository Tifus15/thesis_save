...
\end{equation}
We will consider this system as one dimensional $\mathbf{x}= x$ along straight line which produces simple harmonic motion \cite{osci}. It is built with the spring  stiffness $k$ and mass connected to it $m$. To show the simple harmonic motion let us solve the equation.\\
...
\subsection{Analytical solution of Ordinary Differential Equations}\label{ana}
...
The Consistency are proven over local discretization error.
Let say that for $(x,y)\in G \mu = \mu(\eta)$ is a solution for the ODE $\mu^{'} = f(\eta,\mu), \mu(x)=y$,then the discretisation error is
\begin{equation}
...
\begin{equation}
	\dot{\mathbf{x}} = \mathbf{J}\frac{\partial\mathcal{H}}{\partial \mathbf{x}}(x)
\end{equation}
...
\begin{equation}
	\left[\mathbf{J}-\mathbf{R}\right]\frac{\partial\mathcal{H}}{\partial x}(x) + \mathbf{g}(x)u.
\end{equation}
...
The main goal of feedforward networks is to fit the model so that the we get best approximation. Fit the model means to learn the parameters of the model trough optimizing procedure.\\
...
Recurrent Neural Networks are very interesting concept of Neural Network architecture. It is mostly used for speech recognition but for a time series\cite{mlprnn}.\\
